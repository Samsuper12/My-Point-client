...
//        return QVariant(item.done);
    ...
    case Danger:
        return QVariant(item.danger);
    case Userlogin:
        return QVariant(item.userLogin);
...
void ToDoModel::setList(ToDoList *list)
{
    ...
    if (mList) {
        ...
        connect(mList, &ToDoList::preAllItemAppended, this, [=]() {
            const int index = mList->items().size();
            beginInsertRows(QModelIndex(), 0, index);
        });

        connect(mList, &ToDoList::postAllItemAppended, this, [=]() {
            endInsertRows();
        });
